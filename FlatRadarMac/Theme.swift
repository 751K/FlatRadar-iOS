...
enum Theme {
    ...
    /// 两张迷你图的颜色：统计带的趋势线、Alerts 的 24 小时柱子。
    ///
    /// 就是 iOS 那条曲线的颜色
    /// ---------------------
    /// iOS 的 `SparklineView.tint` 默认值是 `.blue`，而 `DashboardView` 里那个唯一的
    /// 调用点没有覆盖它——两端画的是同一件事（每日新增），那就用同一个色。
    ///
    /// 写 `.blue` 而不是抄 hex，是因为这两端**本来就是同一个值**：在两台真机上
    /// 各跑了一次 `UIColor(Color.blue)` / `NSColor(Color.blue)` 解析，结果一致——
    ///
    /// | | 浅色 | 深色 |
    /// |---|---|---|
    /// | iOS 27（iPhone 16 Pro 实测） | `#0088FF` | `#0091FF` |
    /// | macOS 27（本机实测） | `#0088FF` | `#0091FF` |
    ///
    /// 注意这**不是**记熟的那个 `#007AFF` / `#0A84FF`：systemBlue 在 26→27 这一代
    /// 被调亮过。所以更不该抄 hex——抄的那一刻就钉死在某一代系统上，下次 Apple
    /// 再调，iOS 跟着走而 Mac 留在原地，两端就分叉了。
    ///
    /// 这是「不选色相」那条规则的一处例外
    /// -------------------------------
    /// 上面 ``ink`` 的注释里写了：Mac 上任何带色相的强调色都会撞进 7 个平台色 +
    /// 5 个状态色里，systemBlue 尤其会撞 Holland2Stay 的平台蓝。实际差多少：
    ///
    /// | | 图表色 | H2S 平台色 |
    /// |---|---|---|
    /// | 浅色 | `#0088FF` | `#0069D9`（更暗更沉，分得开） |
    /// | 深色 | `#0091FF` | `#0A84FF`（**几乎同一个色**） |
    ///
    /// 明知深色下会撞还这么用，是因为撞的代价在这两张图上不成立：它们不在表格行里，
    /// 也不用颜色编码任何东西（整张图只有一条线、一组同色柱子，颜色不区分谁是谁）。
    /// 会被误读的是"同一行里三个蓝分别在说什么"，这两张图不参与那件事。
    /// 哪天真觉得碍眼，改这一个常量就够，动不到 ``ink`` 管的那十几处。
    static let chart = Color.blue
    ...
    // - **控件里的文字（按钮、勾选框、Toggle、搜索框、分段选择、筛选 token）
    //   → `.body`**，不跟上面那四档走
    // - **文字不写 10pt 以下**；只有 SF Symbol 的装饰性字形可以（箭头、✕）
    //
    // 控件那一档是后补的，原先漏了。上面四档说的是**数据密度**——同一行里谁比谁
    // 重要；控件不在这个坐标系里，它要跟的是 AppKit 的邻居。HIG 把控件字号钉死在
    // regular **13** / small 11 / mini 9（见上），所以一个 12pt 的按钮放在任何一个
    // 系统控件旁边都矮一档。漏掉这一档的后果是搜索框、All filters、Inspector 的
    // 六个动作按钮、FilterPanel 的勾选框全默认掉进了「次级数据」那一档。
    //
