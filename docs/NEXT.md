...
落地的比原计划多一层：除了放回三类 POI，还给选中的房源画了「10 分钟可达」的
同心圆——步行（蓝实线 640m）和骑车（紫虚线 1920m）。POI 告诉你"那儿有个超市"，
圈告诉你"它在不在你走得到的范围里"，两者合起来才回答得了"这地方住着方便吗"。

骑车那圈是荷兰语境下最有用的一个：日常出行默认就是自行车，而它和步行圈差三倍，
车站、大超市、学校往往都在步行圈外、骑车圈内。

半径除了绕路系数 1.3——圆是直线距离，人得沿街走，运河城市尤其绕。宁可画保守。

配套改动：圈的锚点不跟着卡片关闭而消失（关卡片的目的恰恰是看图）；iPad 横屏的
房源卡片从底部 sheet 改成右下角浮层，否则圈全被压在卡片下面。
...
| 3. 地图 POI | **已完成** | 无（还多做了可达圈） |
| — iPad 适配 | **大量已落地**（见上面追记） | 结构性的那半还没动，见下 |
| — Xcode 27 迁移 | **已落地** | 无 |
| — App 图标 | **诊断完成，未修** | 见下 |

**按原计划，2.1.0 在 iOS 这边已经没有欠账了**：1a 客户端做完交给后端，第 3 条
做完，第 2 条本来就留给 2.2.0。实际范围比原计划大得多——iPad 适配和 Xcode 27 迁移
都不在最初那三条里。

剩下的是下面两件悬案，以及 1b（端上模型可用性上报）——后者不做也不影响 2.1.0 发版，
只是会让 2.2.0 的判断继续缺数据。
