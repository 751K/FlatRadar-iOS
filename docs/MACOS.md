...
**文案继续往一份收**（`StatusWording`，在包里）。统计带、菜单栏和三格小组件调同一个
函数。收拢时抓到两处漂移：`StatsStrip` 没套筛选时写 `Showing`、菜单栏同一个数写
`Listings`，而 `MenuBarStatus` 的注释写的又是第三种说法。统一成 `Listings`。
`pytest` 里有一条扫描，小组件源码里再出现裸的 `Text("…")` 就红（品牌名除外）。

**尺寸：设计稿的大号是 360×376，macOS 真正的大号是 329×345**（HIG 尺寸表），矮 31pt。
整屏按同一个比例收了一档（大数字 54→46、柱高 46→36、行高 33→30、段间距 14→10），
而不是砍掉某一段内容——内容层级是设计稿的主张，尺寸是系统给的，该让的是后者。
小号同理：稿子 170，系统 155。
