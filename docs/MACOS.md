...
`FlatRadarMacWidget` 是一个 `com.apple.product-type.app-extension`，
两档尺寸（`systemSmall` / `systemMedium`）。`systemLarge` 不给：没有第三样东西
可放，多出来的空间只会诱使人往里塞那个说好不做的列表。
...
**文案收成一份**（`StatusWording`，在包里）。菜单栏和小组件调同一个函数，
iOS 那个还没做的主屏小组件将来是第三个调用方。写这段时 `MenuBarStatus.swift`
的注释写着「没套就是 `Showing`」，而它正下方的代码返回的是 `Listings`——
注释和代码在同一个屏幕上就已经漂了，这正是要防的。
