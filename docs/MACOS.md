...
docs/NEXT.md 那一行：「上次扫描时间 + 我的匹配数。**不做房源列表**」。
Phase 4 的菜单栏那一格和它是同一件事，文档原话是**「文案和口径要一致」**——
这一节大半是在讲怎么让那句话变成编译器管的事，而不是靠人记得。

`FlatRadarMacWidget` 是一个 `com.apple.product-type.app-extension`，里面**两格**。

**「总共匹配多少」不是锚点。** 第一版把它放成了小号那一格的大数字，做完看了一遍
就发现不对：那个数一天可能一点不变（个人筛选命中的池子本来就稳），拿它当第一眼
要看的东西是浪费那 44pt。列表页顶上那条统计带（`StatsStrip`）早就给过答案，
设计稿 t3 的原话是：
...
所以小组件照搬这个层级，三档尺寸是**同一个层级的三次截断**，不是三种设计：

| Status（What's new） | 放什么 | 为什么到此为止 |
|---|---|---|
| 小 | New today + 涨跌 + 未读胶囊 | 128pt 宽只够一个 44pt 的数，再加一行就没有锚点了 |
| 中 | 再加右边三行小数（匹配数 / 未读 / 全库） | 正好是统计带的结构，横过来的 |
| 大 | 再加 14 天柱子 + 状态变更 | 高度够了才画得下"最近是多还是少"这个形状 |

第二格 **Move-ins（日历）**，中号 / 大号两档：

| | 放什么 |
|---|---|
| 中 | 左栏「下一个能抢的日子」+ 两周合计，右栏两周柱子 |
| 大 | 头条在上，整幅宽的四周柱子，底下四周合计 |

**为什么是日历不是地图。** 两个都考虑过，日历赢在三点：它是纯数字，画出来不需要
任何图片（地图那一格得先有瓦片，而这个扩展不联网，只能由 app 渲染成 PNG 写进共享
容器——多一条图片管线和一份缓存失效逻辑）；它每天都在变，而地图上那些点几周才挪
一次；它答得上一个具体问题（"我什么时候能搬进去"），而一张 160pt 见方、上面撒着
几百个点的荷兰地图在那个尺寸下只会糊成一团。

日历那格的口径和日历那一屏（`CalendarPane`）完全一致，尤其是 **bookable**：实测
691 条里 609 条是 Occupied（88%），它们的 `available_from` 是未来的**退租日**，
不是"现在能订"。所以柱子的总高是那天起租的全部，**实心的那一段**才是能动手的；
「下一个 move-in」找的也是 `bookable > 0` 而不是 `total > 0`，否则第一个命中多半
是一堆点进去什么也做不了的退租日。

**日历数据只在那一格真的摆在桌面上时才取。** `MainWindow` 里有一条量过的注释：
`/calendar` 回 691 条、211 KB，是四屏里最少打开的一屏，所以刻意做成按需拉。
为了一个多半没人摆的小组件把那个决定推翻掉，是拿所有人的冷启动带宽换少数人的
一格。改成开机时先问一次系统「这一格装了没有」（`WidgetBridge.isInstalled`，
一次异步 IPC），没装就一个字节都不多要。

**这一格不联网。** 它的数字全部由 app 算好、整份写进 App Group 容器
（`WidgetSnapshot` / `WidgetBridge`），小组件只负责画。理由是让扩展自己取数得先
往外挪三样东西，每一样都是一处新的漂移点：
...
| 「这个数套没套个人筛选」 | 抄一份判断过去 | 就是「文案和口径要一致」禁止的那件事 |

代价只有一个：数据会旧。那一条单独处理，见下。

**文案收成一份**（`StatusWording`，在包里）。统计带、菜单栏和两格小组件调同一个
函数，iOS 那个还没做的主屏小组件将来是第四个调用方。收拢的时候**当场抓到两处**：

| | 匹配数那一格的标题（没套筛选时） |
|---|---|
| `StatsStrip` | `Showing` |
| `MenuBarStatusView` | `Listings` |
| 它自己的注释 | 写着「没套就是 `Showing`」，而正下方的代码返回 `Listings` |

同一台机器上的两个位置、同一个数、两个名字。统一成 `Listings`——它说的是那**是
什么**，而 `Showing` 说的是"界面正在干什么"，后者在菜单栏和小组件里根本讲不通。
`pytest` 里有一条扫描，小组件的源码里再出现裸的 `Text("…")` 就红。

⚠️ **旧了之后必须改口，这是最容易做错的一条。**

那一格显示 `scanned 4m ago`，而这句话是拿**当前时刻**去减快照里记的扫描时间。
app 有多久没跑，这句话就偏悲观多少秒，一秒不差。所以：

- 10 分钟以内照说 `scanned X ago`。这个阈值**就是**我们愿意给这句话注入的
  最大误差，不是随手定的数。
- 超过之后改说 `checked X ago`——说我们自己是几时取的，不再替后端发言。

不改口的后果不是"数据旧了"：读者看到桌面上写着 `scanned 3d ago`，读出来的是
**「这个服务三天没扫了」**，而事实是「你这台 Mac 三天没开过 FlatRadar」。
那是替后端背了一口它没犯的锅，而且锅就摆在桌面上。数字本身两种状态都照常显示
（只是压成次要色）——匹配总数比扫描时刻稳得多，一格只会说「请打开 app」的
小组件没有存在的理由。

**登出要清。** 匹配数和未读数是账户数据（风险 6：「统一断流、清空所有窗口的
账户数据」）。窗口里清干净、桌面上那一格还挂着上一个账号的数字，那条判据就没
做完，而且它比窗口显眼。

**刷新从哪来**：app 每次 `loadOnce` / `refreshShared`（冷启动、⌘R、打开菜单栏
面板）写一次，开着窗口时未读数变了也补一次。菜单栏常驻、一个窗口都没有的形态下
要等下一次刷新——有意如此，为一个次要数字装一条常驻写路径不值得，而且它不会
因此说谎，上面那条改口规则管着。小组件自己的时间轴只负责让相对时间往前走
（一小时内每分钟一跳，之后每小时一跳，共 84 个条目管 24 小时）。

三处**静默**的坑，都写了测试钉住：

1. **macOS 的 App Group 名必须带 team 前缀**（`HGXZB3UC25.group.…`）。
   写成 iOS 那样的裸 `group.…` 时 `containerURL(...)` **照样返回一个路径**，
   只有写那一步被沙盒拒掉——症状是那一格永远空着，离原因隔着两层。
   `FlatRadarMacTests/WidgetBridgeTests` 跑在带 entitlement 的宿主 app 里，
   实测过这个变异：五条挂三条。
2. **Info.plist 不能放进 `FlatRadarMacWidget/`**。那是个
   `PBXFileSystemSynchronizedRootGroup`，扔进去的文件自动成为 target 资源，
   Xcode 会直接警告 Copy Bundle Resources 里有它。和 `d7971f5` 把两份 test plan
   挪出 `FlatRadar/` 是同一类事，只是那次的代价大得多。所以它在工程根上，
   叫 `FlatRadarMacWidget-Info.plist`。
3. **appex 要真的被嵌进 `.app`**（Copy Files，`dstSubfolderSpec = 13`
   = `Contents/PlugIns`）。少了这条阶段，appex 照样构建得出来，只是系统不知道
   有这个小组件——构建全绿。
4. **`WidgetKind` 那两个串是桌面上那一格的身份**，改了等于把用户摆好的那格弄没
   （变空白，得手动删了重摆）。值本身钉在 `tests/test_widget_wiring.py` 里。

**每一档尺寸都渲染出来看过，深浅各一遍。** 为此把视图拆成 `…Face` / `…Layout`
两层——`EnvironmentValues.widgetFamily` 是只读的，`previewContext` 在 Xcode 预览
之外不生效，尺寸档不当参数传就根本画不出小号那一版。看出来的问题没有一个是
读代码能发现的：柱子平分整幅宽之后变成一排药丸（`Capsule` 的圆角跟着宽度走）；
未读胶囊在中号里停在整格正中间，看起来像右边那列的东西；日历中号竖排时柱子只
分到 22pt；没有数据时日历大号在报 `Move-ins 0 / Bookable 0`，而 0 是个结论、
我们只是没数据；访客那一版 `Listings 892` 和 `Total listings 892` 是同一个数
显示了两遍。
