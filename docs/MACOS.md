...
设计稿 4b 那张深色卡给的底是 `#16212B`，一个偏蓝的深色。在稿子里好看，真摆到
桌面上就露馅了——旁边的日历、天气那几格是系统材质（中性近黑、半透、透着壁纸），
这几格是一块实心的蓝灰，一眼看出是外人。**这个只有在真桌面上才看得出来**，
渲染脚本里每一格都是单独一张图，没有邻居可比。

所以深色走 `.fill.tertiary`（就是系统给小组件的那层材质），和旁边那些格子按构造
一致，壁纸换了也跟着变。浅色仍然用稿子那个暖纸底：浅色下系统材质是近白，和
`#FBFAF7` / `#F3F0E8` 本来就贴得很近，而那点暖是 app 图标里窗户的颜色，值得留着。
