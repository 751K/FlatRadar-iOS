...
**2026-09-20 更新**：iOS / iPadOS 当前已发布 `2.2.0`，原生 macOS 当前已上架 `1.0.1`；
iOS / iPadOS `2.3.0` 与 macOS `1.1.0` 是下一版本，正在开发中。
Mac 客户端开发和首次上架目标已完成；Phase 0 中“将共享路由抽入 Core”仍保留为未完成技术债，
当前不阻塞 Mac 功能或后续版本发布。
`FlatRadarCore/` 现在是本地 SwiftPM 包，iOS 与 macOS 两个 app 都只依赖它的产品。
两端 Debug / Release 均构建通过，测试 175 条（78 app + 97 包）与迁移前一致，
另新增 3 条包资源测试。Mac 的签名、推送链路、截图和分发构建均已按阶段验证。
...
- [x] **菜单栏常驻**：`MenuBarExtra`。面板照设计稿 t5：今日新增 + 趋势条 + 当前筛选
      + 五条可点的最新房源 + 下一个能抢的入住日，底部 `⌘O` / `⌘,` / `⌘Q`。
      **默认开**（改过一次，原先是关的）——这个 app 的用途就是"不打开它也知道有没有
      新房"，而这是唯一在没有窗口时还提供它的形态，藏在设置页第二个开关后面说不通。
      ⚠️ 代价是**装完就有一条常驻 SSE**（它同时决定"没有窗口时还维不维持连接"），
      不再是用户自己按下的。开关仍在设置页 General，关掉之后流也跟着断。
...
⚠️ **深色的底没有照稿子来，而且改对之前错了一版。**

稿子 4b 给的是 `#16212B`，一个偏蓝的深色。在稿子里好看，真摆到桌面上就露馅：
旁边的日历、天气是中性近黑，这几格是蓝灰，一眼看出是外人。**这个只有在真桌面上
才看得出来**——渲染脚本里每一格都是单独一张图，没有邻居可比。

第一次改错了：换成 `.fill.tertiary`，以为那是"系统给小组件的那层材质"。
**那个填充几乎是透明的**，画出来是一块被壁纸透穿的淡紫，比原来更不像。
是把五个候选并排画在同一张紫壁纸上才看出来的，光读文档看不出它有多透——
这也是这一节的方法论：**颜色的问题要并排看**，单看一张永远觉得还行。

第二版才对：仍然自己刷一个不透明的底，深色换成中性的 `#1C1C1E`（Apple 深色下
那一档面色的标准值）。浅色不动——浅色下系统组件是近白，和 `#FBFAF7` / `#F3F0E8`
本来就贴得很近，而那点暖是 app 图标里窗户的颜色，值得留着。
...
**iOS 大号后来收成了四块**（2026-09-17 第二轮）：数字锚点 + 未读胶囊、趋势条、
**一行汇总文字**、最新三条。原来那三格统计（`Live now` / `Unread` / `Matching`）
在 364pt 宽里各占 110pt，里面只装「一个标题 + 一个数」——信息密度很低，而它吃掉的
40pt 正是下面那三条最缺的。收成一行之后，那几条的行高 33→44、标题 11.5→13、
副标题 9.5→11：手机是拿在手里看的，地址和价钱才是要读的东西。
（第三轮又从三条减到**两条**，换来更大的锚点数字。）

那一行**又撞上同一个数据缺口，第三次**。想要的形状是
`831 live · 12 watching · 1 lottery closes in 2d`，后两段都没有数据源：

| 想要的 | 为什么没有 |
|---|---|
| `12 watching` | 没有"关注列表"这个概念。Mac 那边 `BrowseModel.pinned` 是「钉两套并排比」，上限 2、窗口级；iOS 连这个都没有 |
| `1 lottery closes in 2d` | 抽签截止时刻在 openapi 里（`deadline` / `closes` / `draw_at`）**各 0 次**，listings 表只有 `available_from` 而且只到日 |

换成同样形状、真有数的三段：`831 live · 193 matching · 118 this week`
（`new_7d` 在 `/stats/public/summary` 里本来就有，不用多发请求）。没套个人筛选时
`matching` 那段整段不出现——那时它和 `831 live` 是同一个数。

**拿不到的那一段整段不出现**，不写 `— live`：一行里出现一个破折号，读的人得先
判断那是"没取到"还是"真的是零"。

macOS 的大号不动：它只有 345pt，本来就排不下更大的行，三格统计在那个宽度里也不算
浪费。

**手机那一档还去掉了页眉的 `FlatRadar` 字样**：那一格就贴在自家图标旁边，长按还会
显示 app 名字，再写一遍是重复。桌面上留着——通知中心那一栏挤着一排来自不同 app 的
格子，一个 12.5pt 的字比图标好认。时间那一段两端都留，它不能省：底下所有数字都是
「截至那一刻」的，没有它就没法判断新旧。
...
### Phase 5 · 上架（已完成，Mac 1.0.1）

Mac 客户端已完成归档、分发、截图、审核和 App Store 上架。当前已上架版本为 `1.0.1`。
应用内共享本地化、资源正确性和签名配置也已随分发构建验证。

- [x] 推送（见风险 3）
- [x] Mac 图标（`1839ba6` 已经换成 Icon Composer 文档，它支持 macOS 变体）
- [x] Mac 截图 + 单独审核

**完成判据**：若发布，归档及分发构建可安装启动，通过会话与推送冒烟验证；截图、元数据、语言和
隐私声明与实际功能一致。该判据已由 Mac 1.0.1 上架流程完成。
