...
struct MapView: View {
    ...
    ///
    /// 阈值 1100 是**照着真实设备宽度挑的分界线**，不是拍的：
    ///
    ///     竖屏  744(mini) · 820 · 834 · 1024(12.9)
    ///     横屏  1133(mini) · 1180 · 1194 · 1366(12.9)
    ///
    /// 1100 干净地把两组分开。一度写的是 1000（照着 11 寸竖屏 834 算的），那会把
    /// **12.9 寸竖屏的 1024 误判成横屏**——而 Xcode Cloud 的截图正是在大 iPad
    /// 竖屏上跑的，那台设备会拿到本该只在横屏出现的浮层。
    ///
    /// 注意这和 DashboardView 的 `wide`（1000）不是同一个问题：那边问的是"一行
    /// 排得下大数字块 + 三个小数 + 曲线吗"，跟朝向无关；这边问的是"旁边有没有空地
    /// 放一张 380pt 的卡还留得住地图"，实际就是在区分横竖屏。
    ///
    /// 控件尺寸那边继续用 size class（`isRegular`）：那问的是"这个窗口挤不挤"，
    /// 竖屏 iPad 一样该给大按钮。两个问题，两个判据。
    private var usesFloatingCard: Bool { mapWidth >= 1_100 }
