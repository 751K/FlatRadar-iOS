...
/// 自动预订配置）仍走 Web 后台。
nonisolated struct AdminUserSummary: Decodable, Identifiable, Hashable, Sendable {
    let id: String
    let name: String
    let enabled: Bool
...
nonisolated struct AdminFilterSummary: Decodable, Hashable, Sendable {
    let maxRent: Double?
    let minArea: Double?
    let minFloor: Int?
    let cities: [String]
...
nonisolated struct AdminMonitorStatus: Decodable, Sendable {
    let running: Bool
    let pid: Int?
