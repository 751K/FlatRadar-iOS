...
///
/// 两端（FlatRadarTests/SparklineCurveTests）有一份一模一样的测试。改了一边没改另一边，这里会红。
final class SparklineCurveTests: XCTestCase {
    ...
    func test_每段都是三次贝塞尔且段数对得上() {
        let segs = segments(SparklineShape(values: Self.data.map(Double.init)).path(in: Self.rect))
        XCTAssertEqual(segs.count, Self.data.count - 1)
    }

    /// 核心断言：控制点不许跑到这一段两个端点之外。
    func test_控制点不越出本段的数据范围() {
        for (i, seg) in segments(SparklineShape(values: Self.data.map(Double.init)).path(in: Self.rect)).enumerated() {
    ...
    func test_相等的两天之间是平的() {
        let flat = segments(SparklineShape(values: Self.data.map(Double.init)).path(in: Self.rect)).filter { abs($0.from.y - $0.to.y) < 1e-6 }
    ...
    /// 顺带钉住上下余量：和另一端一样是 4。
    func test_上下余量是4() {
        let ys = segments(SparklineShape(values: Self.data.map(Double.init)).path(in: Self.rect)).flatMap { [$0.from.y, $0.to.y] }
