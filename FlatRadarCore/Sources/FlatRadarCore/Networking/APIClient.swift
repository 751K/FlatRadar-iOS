...
public final class APIClient {
    ...
    public static let defaultServerHost = "flatradar.app"
    ...
    public func logout() async throws -> RevokePayload {
    ...
    public func uploadCrashDiagnostic(
        kind: String,
        payload: [String: Any]
    ) async throws {
        // 元信息（设备型号 / iOS 版本）走 body 而不是 header —— 便于后端 grep 时
        // 直接看到，不用解析 User-Agent。
        let platform = PlatformEnvironment.info
        let device = platform.hardwareModel
        let iosVersion = platform.systemVersion
    ...
    public func changePassword(current: String, new: String) async throws -> ChangePasswordResponse {
    ...
    public func verifyPassword(_ password: String) async throws -> VerifyPasswordResponse {
    ...
    public func getPublicSummary() async throws -> MonitorStatus {
    ...
    public func getPublicChart(key: String, days: Int = 30) async throws -> ChartData {
        try await request(
    ...
    public func getListings(city: String? = nil, status: String? = nil,
    ...
    public func getNotifications(limit: Int = 50, offset: Int = 0) async throws -> NotificationsResponse {
    ...
    public func getFilterOptions() async throws -> FilterOptions {
        try await request("GET", "api/v1/filter/options")
    }

    // MARK: - Legal

    public func getLegal(lang: String? = nil) async throws -> LegalResponse {
    ...
    public func testPush(title: String? = nil,
    ...
    public func deleteAccount() async throws -> AccountDeleteResponse {
        try await request("DELETE", "api/v1/me")
    }

    // MARK: - Feedback

    public nonisolated struct FeedbackBody: Encodable {
        public let kind: String      // "bug" | "suggestion" | "other"
        let message: String
        let user_name: String
        let app_version: String
    }

    public nonisolated struct FeedbackResponse: Decodable, Sendable {
        let submitted: Bool
    }

    public func submitFeedback(kind: String, message: String,
                        userName: String = "",
    ...
    public func meExport() async throws -> Data {
...
public nonisolated struct RevokePayload: Decodable , Sendable {
    let revoked: Bool
}
