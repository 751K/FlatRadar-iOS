...
public nonisolated enum ReviewPromptPolicy {
...
public final class ReviewPromptStore {
    ...
    /// `defaults` 的默认值让宿主可以 `ReviewPromptStore()`，包测试又能注入替身。
    /// 公开的是这一个 init，不另加空 init——那会让 `defaults` 没机会赋值。
    ...
    public func noteActiveDay(hasNotifications: Bool, now: Date = Date()) {
    ...
    public func noteListingOpened() {
        defaults.set(defaults.integer(forKey: Key.listingOpens) + 1, forKey: Key.listingOpens)
    }

    /// 现在该不该问。
    public func shouldAsk(_ trigger: ReviewPromptPolicy.Trigger, now: Date = Date()) -> Bool {
        // 截图自动化下**绝不**弹。
        //
    ...
    public func markAsked(now: Date = Date()) {
