...
/// 而不是断言"对所有平台生效"。
public nonisolated enum PlatformScope {

    public nonisolated struct Note {
    ...
    public static func note(appliesTo: [String], selectedSources: [String]) -> Note? {
        ...
        if !selectedSources.isEmpty {
            ...
            if effective.isEmpty {
                return Note(
                    ...
                    // ——这三句原先正是这样。宁可让这行长一点。
                    text: String(localized: "None of the platforms you selected support this filter, so it currently has no effect.", bundle: .module),
                    isWarning: true)
            }
            if effective.count < chosen.count {
                let list = effective.map { Platform.displayName($0) }.sorted()
                return Note(
                    text: String(localized: "Of the platforms you selected, this only filters \(sentenceList(list)). The rest are not affected.", bundle: .module),
                    isWarning: false)
            }
            return nil
        }
        ...
        return Note(
            text: String(localized: "Only filters \(sentenceList(names)). Listings from other platforms are not affected.", bundle: .module),
            isWarning: false)
    }
