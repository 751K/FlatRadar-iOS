...
public nonisolated struct ChartData: Decodable, Sendable {
    public let key: String
    public let days: Int
    public let data: [ChartEntry]

    /// 逐成员构造器随 `public` 变 internal。ChartDetailView 要用它按语义合并后
    /// 重造一份（type_dist / energy_dist 的分桶），所以显式公开。
    public init(key: String, days: Int, data: [ChartEntry]) {
        self.key = key
        self.days = days
        self.data = data
    }
...
public nonisolated struct ChartEntry: Decodable, Identifiable, Sendable {
...
extension Array where Element == ChartEntry {
    ...
    ///                 B/C/D/E/F/G 原样。
    public nonisolated func bucketed(forKey key: String) -> [ChartEntry] {
        switch key {
        case "source_dist":
